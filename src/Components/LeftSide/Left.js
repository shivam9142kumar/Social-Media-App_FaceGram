          ...
          <div className="user-name-userid">
            <img src={profileImg ? (profileImg) : Profile} alt="" />
              <div className='L-user'>
                <h1>{modelDetails ? (modelDetails.ModelName) : "Shivam"}</h1>
                <span>{modelDetails ? (modelDetails.ModelUserName) : "@shivam9142"}</span>
            </div>
          </div>
        </Link>
        <MoreHorizIcon onClick={()=>setLogOutExit(!logOutExit)} className='vert'/>
          ...
          {logOutExit && (
            <div className="logOutExitContainer">
              ...
              <Link to="/" style={{width:"100%"}}><button>Log out @shivam9142</button></Link>
            </div>
          )}
      </div>
